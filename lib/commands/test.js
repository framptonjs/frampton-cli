...
Test.prototype.setupTests = function(outputPath) {
  ...
  const qunitPath = path.join(cliNodeModules, '/qunitjs/qunit');

  if (fs.existsSync(qunitPath)) {
    extra.copySync(qunitPath, path.join(outputPath, 'qunit'));
  } else {
    extra.copySync(
      path.join(projectNodeModules, '/qunitjs/qunit'),
